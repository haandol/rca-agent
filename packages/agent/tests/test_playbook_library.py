...
from rca_agent.adapters.secondary.playbook.library import _pack, encoded
...
def test_retrospective_requires_matching_baseline_and_original_commit(storage):
    ...
    with pytest.raises(ValueError, match="baseline changed"):
        store._library.stage(
            revised, "rca-1", "retrospective:exec-1", baseline=dict(baseline, library_revision="proposal:stale")
        )
    assert store._library.head("pb-1") == head
    pending = store._library.stage(revised, "rca-1", "retrospective:exec-1", baseline=baseline)
    assert pending["publication_status"] == "PENDING"
    assert store._library.head("pb-1") == head
    with pytest.raises(ValueError, match="revision not committed"):
