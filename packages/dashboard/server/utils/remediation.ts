...
function readMetricsNormalized(verificationStatus: string, ...values: unknown[]): boolean | null {
...
function readStringArray(...values: unknown[]): string[] {
  const value = values.find(candidate => Array.isArray(candidate) && candidate.every(item => typeof item === 'string'))
...
export function readSessionRemediation(item: DataRecord): RemediationDetails {
  ...
  return {
    ...
    verificationStatus,
    metricsNormalized: readMetricsNormalized(verificationStatus, item.metrics_normalized, item.metricsNormalized),
    verificationSummary: readString(item.verification_summary),
    remainingIssues: readStringArray(item.verification_remaining_issues, item.remaining_issues),
    remediationFaultType: readString(item.remediation_fault_type, item.fault_type),
    remediationEndpoint: readString(item.remediation_endpoint, item.endpoint_path),
  }
}
...
export function readSpanRemediation(item: DataRecord): RemediationDetails {
  ...
  const verificationStatus = readString(verification?.status, metadata?.verification_status, item.verification_status)

  return {
    remediationStatus: readString(metadata?.status, metadata?.remediation_status, item.remediation_status),
    remediationSuccess: readBoolean(metadata?.success, metadata?.remediation_success, item.remediation_success),
    remediationSummary: readString(metadata?.summary, metadata?.remediation_summary, item.remediation_summary, item.input_summary),
    remediationError: readString(metadata?.error, item.remediation_error, item.error),
    remediationCompletedAt: readString(metadata?.completed_at, item.remediation_completed_at, item.end_time),
    verificationStatus,
    ...
    verificationSummary: readString(verification?.reason, verification?.summary, metadata?.verification_summary, item.verification_summary),
    ...
    remediationFaultType: readString(metadata?.fault_type, metadata?.remediation_fault_type, item.remediation_fault_type, item.fault_type),
    remediationEndpoint: readString(metadata?.endpoint_path, metadata?.remediation_endpoint, item.remediation_endpoint, item.endpoint_path),
  }
}
...
export function mergeRemediationDetails(
  ...
  return {
    ...
    remediationSummary: primary.remediationSummary || fallback.remediationSummary,
    remediationError: primary.remediationError || fallback.remediationError,
    remediationCompletedAt: primary.remediationCompletedAt || fallback.remediationCompletedAt,
    ...
    verificationSummary: primary.verificationSummary || fallback.verificationSummary,
    remainingIssues: primary.verificationStatus || primary.remainingIssues.length
      ? primary.remainingIssues
      : fallback.remainingIssues,
    remediationFaultType: primary.remediationFaultType || fallback.remediationFaultType,
    remediationEndpoint: primary.remediationEndpoint || fallback.remediationEndpoint,
  }
}
