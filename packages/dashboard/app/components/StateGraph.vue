...
const STATE_DESC: Record<string, string> = {
  ...
  HYPOTHESIS_VALIDATION:
    '수집된 증거를 바탕으로 가설을 채택/기각/추가조사로 분류하는 단계. 진입 시 Accepted Review Gate가 기존 채택 가설을 먼저 리뷰한다.',
  REPORT_GENERATION:
    '채택된 근본원인과 증거를 기반으로 RCA 보고서를 생성하는 단계',
  ANALYZING:
    'CC Headless가 RCA, 조건부 복구, 보고서 전문 에이전트를 순서대로 실행하는 단계',
...
const pipelineStates = computed(() =>
  happyPath.value.filter((s) => !TERMINAL_STATES.includes(s)),
);
    ...
    <div class="w-56 shrink-0 overflow-y-auto">
      <template v-if="selectedState">
        ...
        <div v-if="pipelineStates.includes(selectedState)" class="mt-2">
          ...
          <div class="flex flex-wrap gap-1">
            <span
              v-for="t in TERMINAL_STATES.filter((s) => s !== 'COMPLETED')"
              :key="t"
