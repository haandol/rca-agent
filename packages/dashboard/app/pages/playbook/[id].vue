  ...
  <div class="space-y-5">
    ...
    >
      ...
      <div>
        ...
        <div class="text-sm font-mono mt-1">
          {{ playbook ? formatDuration(playbook.durationMs) : '-' }}
        </div>
      </div>
    </div>
    ...
    <template v-else-if="playbook && playbook.spanStatus === 'FAILED'">
      <div class="bg-base-100 rounded-xl border border-error/20 p-6">
        ...
        <div
          v-if="playbook.error"
          class="text-sm bg-error/5 text-error rounded-lg p-4 font-mono break-words"
        >
          {{ playbook.error }}
        </div>
        <p v-else class="text-sm text-base-content/60">
          플레이북 생성 중 오류가 발생했습니다.
        </p>
      </div>
    </template>
    ...
    <template v-else-if="playbook">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="space-y-4">
          ...
          <div
            ...
            class="bg-base-100 rounded-xl border border-base-content/5 p-5"
          >
            <div
              class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-2"
            >
              장애 유형
            </div>
          ...
          >
            ...
            <div
              ...
              v-html="md(playbook.symptom_pattern)"
            />
          </div>

          <!-- Severity criteria -->
          <div
            v-if="playbook.severity_criteria"
            class="bg-base-100 rounded-xl border border-base-content/5 p-5"
          >
            <div
              class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-2"
            ...
            <div
              class="prose prose-sm max-w-none"
              v-html="md(playbook.severity_criteria)"
            />
          </div>
          ...
          >
            ...
            <div
              ...
              :key="i"
              class="prose prose-sm max-w-none [&:not(:last-child)]:mb-3"
            >
              <div v-html="md(step)" />
            </div>
          </div>
          ...
          <!-- Related metrics -->
          <div
            v-if="playbook.related_metrics?.length"
            class="bg-base-100 rounded-xl border border-base-content/5 p-5"
          >
            <div
              class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-2"
            ...
            <div
              ...
              :key="i"
              class="prose prose-sm max-w-none [&:not(:last-child)]:mb-2"
            >
              <div v-html="md(m)" />
            </div>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div
            v-if="playbook.temporary_mitigation"
            class="bg-warning/5 rounded-xl border border-warning/15 p-5"
          >
            <div
              class="text-[11px] font-medium text-warning uppercase tracking-wider mb-2"
            >
              임시 완화 조치
            </div>
            <div
              class="prose prose-sm max-w-none"
              v-html="md(playbook.temporary_mitigation)"
            />
          </div>

          <!-- Permanent remediation -->
          <div
            v-if="playbook.permanent_remediation"
            class="bg-success/5 rounded-xl border border-success/15 p-5"
          >
            <div
              class="text-[11px] font-medium text-success uppercase tracking-wider mb-2"
            >
              영구 복구 방안
            </div>
            <div
          ...
          <!-- Escalation criteria -->
          <div
            v-if="playbook.escalation_criteria"
            class="bg-error/5 rounded-xl border border-error/15 p-5"
          >
            <div
              class="text-[11px] font-medium text-error uppercase tracking-wider mb-2"
            >
              에스컬레이션 기준
            </div>
            <div
              class="prose prose-sm max-w-none"
              v-html="md(playbook.escalation_criteria)"
            />
          </div>
      ...
      <!-- Playbook ID footer -->
      <div
        v-if="playbook.playbook_id"
        class="text-[11px] text-base-content/30 font-mono text-right"
      >
      ...
      <div
        ...
        "
        class="bg-base-100 rounded-xl border border-base-content/5 p-6"
      >
        <div
        ...
        >
          ...
          <p class="text-[11px] mt-1">
            이 세션은 메타데이터 기록 기능 배포 전에 실행되었을 수 있습니다.
          </p>
          <p
            v-if="playbook.outputSummary"
            class="text-xs font-mono mt-3 text-base-content/50"
          >
            {{ playbook.outputSummary }}
          </p>
        </div>
      </div>
    </template>
