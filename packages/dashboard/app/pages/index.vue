...
import { STATE_DESC, STATE_LABEL, isTerminalState } from '~/utils/sessionState';
...
const stats = computed(() => {
  ...
  const inProgress = sessions.value.filter(
    (s) => !isTerminalState(s.state),
  ).length;
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-base-content/50 mt-0.5">
          Root Cause Analysis 파이프라인 실행 이력
        </p>
      </div>
      <button
    ...
    <div v-if="stats" class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card">
        <div class="text-xs font-medium text-success uppercase tracking-wider">
          완료
        </div>
      ...
      <div class="stat-card">
        <div class="text-xs font-medium text-error uppercase tracking-wider">
          실패
        </div>
    ...
    <!-- Sessions Table -->
    <div
      class="bg-base-100 rounded-xl border border-base-content/5 overflow-hidden"
    >
      <div
        v-if="status === 'pending' && !sessions"
      ...
      <table v-else class="table w-full">
        <thead>
          <tr class="border-b border-base-content/5">
            <th class="sortable-th pl-5" @click="toggleSort('state')">
              상태
              <span v-if="sortKey === 'state'" class="sort-indicator">{{
                sortDir === 'asc' ? '▲' : '▼'
              }}</span>
            </th>
            <th class="sortable-th" @click="toggleSort('alarmName')">
              알람
              <span v-if="sortKey === 'alarmName'" class="sort-indicator">{{
                sortDir === 'asc' ? '▲' : '▼'
              }}</span>
            </th>
            <th class="sortable-th" @click="toggleSort('engine')">
              엔진
              <span v-if="sortKey === 'engine'" class="sort-indicator">{{
                sortDir === 'asc' ? '▲' : '▼'
              }}</span>
            </th>
            <th
        ...
        <tbody>
          <tr
            ...
            :key="`${session.rcaId}-${session.engine}`"
            class="session-row border-b border-base-content/5 last:border-0"
          >
            <td class="pl-5">
              <span
              ...
              >
                <span
                  ...
                  :class="
                    stateStyle[session.state]?.dot || 'bg-base-content/30'
                  "
                />
            ...
            <td>
              ...
              <div class="text-[11px] text-base-content/35 font-mono mt-0.5">
                {{ session.rcaId.slice(0, 8) }}
              </div>
            </td>
            <td>
            ...
            <td class="max-w-xs">
              <p v-if="session.rootCause" class="text-sm truncate">
                {{ session.rootCause }}
              </p>
              <p
                v-else-if="session.errorReason"
                class="text-sm text-error/80 truncate"
              >
                {{ session.errorReason }}
              </p>
            ...
            <td>
              <div class="flex items-center justify-end gap-0.5 pr-2">
                <NuxtLink
                  :to="`/trace/${session.rcaId}?engine=${session.engine}`"
                >
                ...
                <NuxtLink
                  :to="`/report/${session.rcaId}?engine=${session.engine}`"
                >
                ...
                <NuxtLink
                  :to="`/playbook/${session.rcaId}?engine=${session.engine}`"
                >
                ...
                <button
                  v-if="!isTerminalState(session.state)"
                  class="action-btn text-warning"
                  title="중단"
                  @click="openCancelModal(session.rcaId, session.engine)"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="size-3.5"
                ...
                <button
                  class="action-btn text-error/70"
                  title="삭제"
                  @click="openDeleteModal(session.rcaId, session.engine)"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="size-3.5"
      ...
      <div class="modal-box max-w-sm">
        ...
        <p class="py-4 text-sm text-base-content/70">
          ...
          <span class="font-mono font-medium text-base-content">{{
            cancelTarget?.rcaId.slice(0, 8)
          }}</span>
          <span class="font-mono text-xs text-base-content/50"
            >({{ cancelTarget?.engine }})</span
          >의 RCA 파이프라인을 중단합니다.<br />
          다음 단계 전환 시점에 파이프라인이 종료됩니다.
        </p>
        <div class="modal-action">
          <button class="btn btn-ghost btn-sm" @click="closeCancelModal()">
            닫기
          </button>
          <button
    ...
    <dialog ref="deleteModalRef" class="modal">
      <div class="modal-box max-w-sm">
        ...
        <p class="py-4 text-sm text-base-content/70">
          세션
          <span class="font-mono font-medium text-base-content">{{
            deleteTarget?.rcaId.slice(0, 8)
          }}</span>
          <span class="font-mono text-xs text-base-content/50"
            >({{ deleteTarget?.engine }})</span
          >의 데이터가 삭제됩니다. 이 작업은 되돌릴 수 없습니다.
        </p>
        <div class="modal-action">
          <button class="btn btn-ghost btn-sm" @click="closeDeleteModal()">
            취소
          </button>
          <button
      ...
      <div class="modal-box max-w-lg">
        ...
        <div class="space-y-2 max-h-96 overflow-y-auto">
          <div
            ...
            :class="
              stateModalTarget === state
                ? 'bg-base-content/5 ring-1 ring-base-content/10'
                : ''
            "
          >
            <span
            ...
            >
              <span
                class="size-1.5 rounded-full"
                :class="stateStyle[state]?.dot || 'bg-warning'"
              />
