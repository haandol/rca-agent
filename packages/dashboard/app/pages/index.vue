...
const STATE_DESC: Record<string, string> = {
  ...
  HYPOTHESIS_VALIDATION: '수집된 증거를 바탕으로 가설을 채택(CONFIRMED), 기각(REJECTED), 또는 추가 조사(NEEDS_INVESTIGATION)로 분류하는 단계. 매 루프 진입 시 Accepted Review Gate가 기존 채택 가설을 리뷰하여 중복 탐색을 차단한다.',
  REPORT_GENERATION: '채택된 근본원인과 증거를 기반으로 한글 RCA 보고서를 생성하는 단계',
