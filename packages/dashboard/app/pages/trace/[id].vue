  ...
  <div class="space-y-4">
    ...
    <template v-else-if="trace">
      ...
      >
        <!-- Flow Canvas -->
        <div
          class="bg-base-100 rounded-xl border border-base-content/5 flex-1 overflow-hidden"
        >
          <VueFlow
            :nodes="graph.nodes"
            :edges="graph.edges"
        ...
        <!-- Detail Panel -->
        <div
          class="bg-base-100 rounded-xl border border-base-content/5 w-80 shrink-0 overflow-y-auto"
        >
          <div class="p-4">
            <template v-if="selectedNode">
              ...
              >
                ...
                <div class="flex gap-2 mt-2">
                  ...
                  >
                  <span
                    v-if="selectedNode.durationMs"
                    class="badge badge-sm badge-ghost font-mono"
                    >{{ formatDuration(selectedNode.durationMs) }}</span
                  >
                </div>
                <div v-if="selectedNode.spanId" class="mt-2">
                  <div
                    class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-0.5"
                  >
                    Span ID
                  </div>
                  <div
                    class="text-[10px] font-mono text-base-content/50 truncate select-all"
                    :title="selectedNode.spanId"
                  >
                    {{ selectedNode.spanId }}
                  </div>
                </div>
                ...
                <div v-if="selectedNode.metadata.failure_type" class="mt-3">
                  <div
                    class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-1"
                  >
                    장애 유형
                  </div>
                  <span class="badge badge-sm badge-outline">{{
                    selectedNode.metadata.failure_type
                  }}</span>
                </div>
                ...
                <div v-if="selectedNode.metadata.symptom_pattern" class="mt-3">
                  ...
                  <div
                    class="prose prose-xs max-w-none bg-base-200/60 rounded-lg p-3 break-words"
                    v-html="md(selectedNode.metadata.symptom_pattern as string)"
                  />
                </div>
                ...
                >
                  ...
                  <div
                    ...
                    v-html="
                      md(
                        (selectedNode.metadata.verification_steps as string[])
                          .map((s, i) => `${i + 1}. ${s}`)
                          .join('\n'),
                      )
                    "
                ...
                <div
                  v-if="selectedNode.metadata.temporary_mitigation"
                  class="mt-3"
                >
                  <div
                    class="text-[11px] font-medium text-warning uppercase tracking-wider mb-1.5"
                  >
                    임시 완화
                  </div>
                  <div
                    class="prose prose-xs max-w-none bg-warning/5 border border-warning/10 rounded-lg p-3 break-words"
                    v-html="
                      md(selectedNode.metadata.temporary_mitigation as string)
                    "
                  />
                </div>

                <div
                  v-if="selectedNode.metadata.permanent_remediation"
                  class="mt-3"
                >
                  <div
                    class="text-[11px] font-medium text-success uppercase tracking-wider mb-1.5"
                  >
                    영구 복구
                  </div>
                  <div
                    class="prose prose-xs max-w-none bg-success/5 border border-success/10 rounded-lg p-3 break-words"
                    v-html="
                      md(selectedNode.metadata.permanent_remediation as string)
                    "
                  />
                </div>
                ...
                >
                  ...
                  <div
                    ...
                    v-html="
                      md(
                        (selectedNode.metadata.prevention_measures as string[])
                          .map((s, i) => `${i + 1}. ${s}`)
                          .join('\n'),
                      )
                    "
                ...
                <div
                  v-if="
                    (selectedNode.metadata.tags as string[] | undefined)?.length
                  "
                  class="mt-3"
                >
                  <div
                  ...
                  <div class="flex flex-wrap gap-1">
                    <span
                      v-for="tag in selectedNode.metadata.tags as string[]"
                      :key="tag"
                ...
                <div
                  v-if="selectedNode.metadata.playbook_id"
                  class="mt-3 pt-3 border-t border-base-content/5"
                >
                  <div
                    class="text-[11px] font-medium text-base-content/40 uppercase tracking-wider mb-1"
                  >
                  ...
                  >
                    {{ selectedNode.metadata.playbook_id }}
                  </div>
                </div>
                ...
                <div v-if="selectedNode.error" class="mt-3">
                  ...
                  <div
                    class="prose prose-xs max-w-none bg-error/5 text-error rounded-lg p-3 break-words"
              ...
              <template v-else-if="selectedNode.nodeType === 'span'">
                ...
                <div v-if="selectedNode.metadata" class="mt-3">
                  ...
                  <div class="bg-base-200/60 rounded-lg p-3 space-y-1.5">
                    <div
                      v-for="(val, key) in selectedNode.metadata"
                      :key="String(key)"
              ...
              <template v-else>
                <div class="flex items-center gap-2 flex-wrap">
                  <span
                    v-if="selectedNode.category"
                    class="badge badge-sm"
                    :class="{
                      'badge-info': selectedNode.category === 'DEPLOYMENT',
                      'badge-warning':
                        selectedNode.category === 'INFRASTRUCTURE',
                  ...
                  <span
                    ...
                    :class="{
                      ...
                      'badge-warning':
                        selectedNode.status === 'NEEDS_INVESTIGATION',
                    }"
                    >{{
                      hypoStatusLabel[selectedNode.status as string] ||
                      selectedNode.status
                    }}</span
                  >
                  <span
                    ...
                    :class="
                      selectedNode.confidenceScore >= 0.8
                        ? 'text-success'
                        : selectedNode.confidenceScore >= 0.5
                          ? 'text-warning'
                ...
                <div
                  v-if="selectedNode.title"
                  class="mt-3 text-sm font-semibold leading-tight"
                >
                  {{ selectedNode.title }}
                </div>
                ...
                <div
                  ...
                  v-html="md(selectedNode.description || selectedNode.label)"
                />
                ...
                <div v-if="selectedNode.evidenceSummary" class="mt-3">
                  ...
                  <div
                    class="prose prose-xs max-w-none bg-base-200/60 rounded-lg p-3 break-words"
                    v-html="md(selectedNode.evidenceSummary)"
                  />
                </div>
                ...
                <div v-if="selectedNode.judgmentReasoning" class="mt-3">
                  ...
                  <div
                    class="prose prose-xs max-w-none bg-base-200/60 rounded-lg p-3 break-words"
                    v-html="md(selectedNode.judgmentReasoning)"
                  />
                </div>
              </template>
            </template>
            ...
            <template v-else>
              <div
                class="flex flex-col items-center justify-center h-full text-base-content/30 gap-3"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="size-8"
    ...
    <dialog ref="evidenceModalRef" class="modal">
      <div class="modal-box max-w-3xl max-h-[80vh]">
        ...
        <div
          v-else
          class="prose prose-sm max-w-none overflow-y-auto max-h-[60vh]"
