...
export function buildTraceGraph(
  ...
  const hypoGenSpan = visible.find(
    (s) => s.spanType === 'HYPOTHESIS_GENERATION',
  );
  if (hypoGenSpan) {
    const rootHypos = hypotheses.filter(
      (h) => !h.parentId || !hypoById.has(h.parentId),
    );
  ...
  if (hypoGenSpan && reportSpan) {
    ...
    const directEdgeIdx = edges.findIndex(
      (e) =>
        e.source === `span-${hypoGenSpan.spanId}` &&
        e.target === `span-${reportSpan.spanId}`,
    );
