...
type SynthesizedTemplates = {
  app: cdk.App;
  codexHeadlessStack: CodexHeadlessStack;
  codexHeadless: Template;
  healthcare: Template;
};
...
function synthesize(): SynthesizedTemplates {
  ...
  const stack = new CodexHeadlessStack(app, 'CcHeadlessTest', {
    vpc,
    alarmTopic,
    notificationTopic,
  ...
  return {
    app,
    codexHeadlessStack: stack,
    codexHeadless: Template.fromStack(stack),
...
test('the analysis task is not told how to reach the service it investigates', () => {
  ...
  const { codexHeadless } = synthesize();

  expect(JSON.stringify(codexHeadless.toJSON())).not.toContain('HEALTHCARE_');
});

test('Codex queue visibility exceeds the analysis and staleness boundary', () => {
  const { codexHeadless } = synthesize();
  const queues = Object.values(
    codexHeadless.findResources('AWS::SQS::Queue'),
  ) as CfnResource[];
...
test('Codex uses the global GPT-5.6 Sol profile at high reasoning', () => {
  ...
  codexHeadless.hasResourceProperties('AWS::ECS::TaskDefinition', {
...
test('Codex task role can exchange AWS credentials for a Bedrock bearer token', () => {
  const { codexHeadless } = synthesize();
  const actions = taskRoleStatements(codexHeadless)
...
test('Codex task role has no ECS service write permissions', () => {
  const { codexHeadless } = synthesize();
  const statements = taskRoleStatements(codexHeadless);
