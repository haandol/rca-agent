...
from headless_codex.adapters.secondary.playbook.library import payload as domain_payload
from headless_codex.services.analysis_parts import json_bytes, validate_recovery_operations
...
def _digest(value) -> str:
    """Hash the exact canonical JSON domain independently of transport formatting."""
...
def preserve_recovery_procedure(public, part: dict | None, rca_id: str):
    ...
    result["execution_steps"] = deepcopy(private["execution_steps"]) if private else []
    result["rollback_context"] = deepcopy(private["rollback_context"]) if private else None
    result["verification_status"] = "DRAFT"
    if private and procedure_identity(result) != procedure_identity(private):
...
def bind_recovery_publication(library, public, part: dict, claim_token: str) -> bool:
    ...
    if private is None:
        return True
    if procedure_identity(private) != procedure_identity(public):
    ...
    if not snapshot or json.loads(snapshot["playbook_json"]) != domain_payload(public):
