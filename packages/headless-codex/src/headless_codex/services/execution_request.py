...
_ENGINES = frozenset({"strands", "headless-codex", "codex-headless", "cc-headless"})
...
def parse_execution_request(message_body: str) -> ExecutionRequest:
    ...
    if engine not in _ENGINES:
        raise InvalidExecutionRequestError(
            "execution request engine must be strands, headless-codex, or legacy codex-headless/cc-headless"
        )
