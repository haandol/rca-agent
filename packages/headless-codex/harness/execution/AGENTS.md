...
않고, 절차에 있는 단계를 임의로 건너뛰지 않는다. 각 단계는 승인된 완성 문자열 `commands`, 고정 인자 `deployment_wait` 또는 `metric_wait`를 가진다.
...
region, max_wait_seconds=900, latency_alarm_name='', completed_work_evidence=None, deployment_step_id='')` 지침을 따른다.
...
서버의 취소 가능한 대기는 최대 900초이며 남은 기존 실행 예산과 MCP timeout 1200초 안에
