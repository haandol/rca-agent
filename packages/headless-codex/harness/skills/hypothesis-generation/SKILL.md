...
- **한 가설은 독립적으로 반증할 수 있는 한 메커니즘**이다. 풀 설정 회귀와 연결 반환 누락처럼
  반증 조건이 다른 원인을 "회귀 또는 누수" 한 항목으로 묶지 않는다. 기존 3~5개 범위 안에서
  증거에 관련된 원인을 구별하며, 평가 기대값을 추측해 가설이나 증거를 만들지 않는다.
- `slow-query`는 비효율 SQL 실행이나 호출 증폭처럼 SQL 작업 자체가 늘거나 느려진 원인이다.
  외부 트랜잭션 잠금 때문에 같은 SQL이 대기한 경우는 `unsupported`다. 풀 설정 회귀도
  연결 반환 누락 증거 없이 `db-leak`로 분류하지 않는다. 유형은 실행 허용 목록이 아니다.
...
```
save_analysis_artifact("hypotheses.json", '{
