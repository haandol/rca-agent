...
2. 현재 의존 서비스 관측에서 대기자와 차단자를 찾는다. DB 관측이 있다면
   ...
   이 이벤트는 서비스 관측자가 출력한다. `activity[]`에 차단 PID가 나타났다는 이유로
   관측자의 서비스 태스크를 차단자에 연결하지 않는다.
   SQL 시간 증가만으로 SQL 작업의 비효율이나 연결 누수를 단정하지 않는다.
3. 차단 PID·트랜잭션 시작 시각·runId/application_name을 `maintenance_lock_acquired` 같은
   소유자 lifecycle 이벤트에 먼저 연결한다. 그 소유자 이벤트의 `@log`/`@logStream`에서
   `ecs_runtime_identity`를 찾는다. DB 스냅샷을 출력한 관측자의 스트림을 대신 사용하지 않는다.
   ...
   `ecs_runtime_identity`의 TaskARN/Cluster/Family/Revision을 그 소유자 이벤트의 `@logStream`에
...
5. production RCA에서 연결된 소유자 이벤트의 `@log`/`@logStream`의 `ecs_runtime_identity`에 실제 기록된
   TaskARN과 Cluster ARN을 `inspect_ecs_task_control(task_arn, cluster_arn)`으로 조회한다.
   반환된 태스크 식별자와 소유 run/journal 태그를 그 소유자 이벤트의 실행과 대조한다.
   연결 근거가 누락되거나 불일치하면 관측자 태스크로 대체하지 않고 소유권 미확인으로 남긴다.
