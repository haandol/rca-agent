...
def test_validation_failure_preserves_partial_artifacts_and_successful_cli_result(
    ...
    from cc_headless.services.artifact_validation import ArtifactValidationError
    ...
    assert diagnostic["ccResult"]["success"] is True
    assert "analysis completed" in diagnostic["ccResult"]["result"]
    assert diagnostic["ccResult"]["rawOutputTruncated"] is True
    assert len(diagnostic["ccResult"]["rawOutput"]) <= eval_adapter._MAX_DIAGNOSTIC_CHARS
...
def test_failed_cli_run_preserves_diagnostics_without_validating_partial_artifacts(
    ...
    class _Runner:
        def run(self, *args, **kwargs):
            ...
            return CcResult(success=False, result="provider unavailable", raw_output="request id: req-1")
    ...
    assert diagnostic["ccResult"]["success"] is False
    assert diagnostic["ccResult"]["result"] == "provider unavailable"
    assert diagnostic["ccResult"]["rawOutput"] == "request id: req-1"
