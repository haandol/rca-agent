...
from codex_headless import eval_adapter
from codex_headless.ports.dto.models import CodexResult
from codex_headless.services.artifact_validation import CompletionArtifacts
...
def test_adapter_rejects_a_scenario_without_an_id(tmp_path):
    ...
    with pytest.raises(SystemExit):
        eval_adapter.main(["codex-headless-eval", str(scenario)])
...
def test_validation_failure_preserves_partial_artifacts_and_successful_cli_result(
    ...
    from codex_headless.services.artifact_validation import ArtifactValidationError
    ...
    with pytest.raises(SystemExit):
        eval_adapter.main(["codex-headless-eval", ""])
...
def test_model_eval_mcp_config_exposes_only_artifact_storage():
    ...
    for name in ("rca-specialist-model-eval.toml", "report-specialist-model-eval.toml"):
        ...
        assert config["mcp_servers"]["rca-progress"]["args"][1] == (
            "{{PACKAGE_ROOT}}/src/codex_headless/mcp_server.py:mcp"
        )
