...
from codex_headless.adapters.secondary.report import s3_report_store
...
def test_report_key_is_isolated_by_attempt_and_claim(monkeypatch):
    ...
    assert key == "reports/codex-headless/rca-1/attempt-3-claim-abc/report.md"
...
def test_completion_notification_does_not_trigger_external_remediation_worker(monkeypatch):
    ...
    S3ReportStore(sns_client=sns).send_notification(
        ...
        "reports/codex-headless/rca-1.md",
        42,
        confirmed=True,
    ...
    assert published["MessageAttributes"]["event_type"] == {
        "DataType": "String",
        "StringValue": "codex_headless_complete",
    }
    assert published["MessageAttributes"]["notification_id"] == {
        "DataType": "String",
        "StringValue": "codex-headless:rca-1:complete",
    }
    body = json.loads(published["Message"])
    assert body["notification_id"] == "codex-headless:rca-1:complete"
...
def test_fifo_completion_notification_uses_stable_sns_deduplication_id(monkeypatch):
    ...
    assert first["MessageDeduplicationId"] == second["MessageDeduplicationId"] == "codex-headless:rca-1:complete"
    assert first["MessageGroupId"] == second["MessageGroupId"] == "codex-headless-completion"
