...
def test_rca_progress_mcp_points_to_packaged_server():
    ...
    assert config["args"][1] == "{{PACKAGE_ROOT}}/src/codex_headless/mcp_server.py:mcp"
...
def test_prompt_resolves_every_include(monkeypatch):
    monkeypatch.setattr("codex_headless.services.prompt_builder._PROMPTS_DIR", PROMPTS_DIR)
...
def test_cc_completion_event_cannot_enter_strands_external_remediation_queue():
    report_store = (
        PACKAGE_ROOT / "src" / "codex_headless" / "adapters" / "secondary" / "report" / "s3_report_store.py"
    ).read_text()

    assert '"StringValue": "codex_headless_complete"' in report_store
...
def test_required_artifact_contract_matches_watcher():
    from codex_headless.services.artifact_watcher import ARTIFACT_SPAN_MAP
...
def test_playbook_guidance_defines_the_execution_step_contract():
    ...
    from codex_headless.services.artifact_validation import _EXECUTION_STEP_FIELDS
...
def test_report_guidance_example_passes_the_completion_gate_rule():
    from codex_headless.services.artifact_validation import _ISO_TIMESTAMP
