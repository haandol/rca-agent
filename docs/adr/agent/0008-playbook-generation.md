...
1. **검색 우선(Search-First) 전략**: 플레이북 생성 전에 S3 Vectors에서 유사 플레이북을 검색한다. 유사도 0.86 이상(`PLAYBOOK_UPDATE_THRESHOLD`)인 기존 플레이북이 있으면 LLM이 새 RCA 결과와 비교하여 업데이트할 내용이 있는지 판단한다. 업데이트가 필요하면 기존 플레이북 ID를 유지한 채 내용을 보강하고, 업데이트가 불필요하면 새 플레이북 생성도 건너뛴다. 일치하는 기존 플레이북이 없을 때만 새로 생성한다.
   ...
   벡터 인덱스는 경량 메타데이터만 보유하므로(결정사항 4), 업데이트 판단 전에 **기존 플레이북의 상세 절차를 실행 트레이스 저장소에서 로드**한다. 로드는 인덱스 메타데이터의 `rca_id`로 해당 RCA 파티션의 플레이북 기록을 조회하는 방식이며, 별도 인덱스를 추가하지 않는다. 상세 로드에 실패하면(레코드 만료·조회 실패) 그 후보는 업데이트 대상에서 **제외**하고 다음 후보로 넘어간다 — 기존 절차를 볼 수 없는 상태의 "업데이트"는 같은 ID로 과거 내용을 덮어써 축적을 되돌리므로, 병합을 포기하고 신규 생성으로 떨어지는 편이 안전하다.
...
2. **벡터 임베딩**: Bedrock **Cohere Embed V4** (`cohere.embed-v4:0`, 1536차원)를 사용하며, 구조화된 템플릿으로 임베딩 텍스트를 생성한다:
   ...
   ```
   각 필드는 80자로 truncate하여 임베딩 품질을 유지한다. 저장 시 `input_type=search_document`, 검색 시 `input_type=search_query`를 사용한다. 동일 템플릿을 RCA 보고서 인덱스(ADR 0001)에도 적용하여 두 인덱스 간 임베딩 공간 일관성을 보장한다.

3. **업데이트 판단 LLM**: `PlaybookUpdateOutput` Pydantic 모델(`needs_update`, 플레이북 전체 필드)을 `structured_output_model`로 지정하여 LLM이 기존 플레이북 대비 새 RCA에서 추가된 점이 있는지 판단한다. `needs_update=false`이면 기존 플레이북을 그대로 유지한다. LLM에는 결정사항 1로 로드한 기존 상세 절차를 함께 제시하며, LLM이 특정 필드를 비워 반환하면 기존 값을 유지한다 — "추가·보강만 하고 삭제하지 않는다"는 병합 규칙을 프롬프트 지시가 아니라 코드로 보장한다.
...
4. **S3 Vectors 인덱싱**: 플레이북 저장 어댑터가 Cohere Embed V4로 `float32` 벡터를 직접 생성하여 저장한다. 메타데이터는 S3 Vectors의 2048 bytes 제한에 맞춰 `failure_type`(80자), `symptom_pattern`(80자), `tags`(CSV 256자), `rca_id`만 포함한다. `verification_steps`, `temporary_mitigation` 등 상세 필드는 DynamoDB의 플레이북 레코드에서 조회한다.
   ...
   따라서 검색 결과 자체로 얻을 수 있는 필드도 이 메타데이터 집합으로 한정되며, 상세가 필요한 소비처(결정사항 1의 업데이트 판단)는 `rca_id`를 경유해 DynamoDB를 한 번 더 조회한다. 벡터 인덱스에 상세를 중복 저장하지 않는 이유는 2048 bytes 제한과, 갱신 시 두 저장소가 어긋날 위험을 피하기 위함이다.
...
- 업데이트 판단을 위한 LLM 호출이 추가로 발생 (기존 플레이북당 1회) + 상세 로드를 위한 DynamoDB 조회 1회
- 플레이북 상세의 수명이 실행 트레이스 보존 주기에 묶인다. 보존 기간이 지난 플레이북은 벡터 인덱스에서 검색되어도 상세를 로드할 수 없어 병합 대상에서 제외되고, 같은 장애 유형의 플레이북이 새 ID로 다시 생성된다

### Risks

- 근본 원인 미확정 RCA에서 생성된 플레이북이 향후 유사 장애 시 오도할 수 있다. "추정 원인 기반" 플레이북으로 명시하고 검증 부족 항목을 표기하여 완화한다.
- 유사도 임계치를 거리로 오해석하면(ADR 0001 참조) 업데이트 경로의 통과 조건이 반전되어, 무관한 플레이북을 병합하거나 반대로 병합이 전혀 일어나지 않는다. 어느 쪽이든 조용히 실패하므로 거리→유사도 변환을 인덱스 어댑터로 단일화하고 계약 테스트로 고정한다.
...
- **스코핑 단계 플레이북 검색**: **제거** (ADR 0001) — 2026-04-28부로 스코핑 경로에서 플레이북 인덱스를 더 이상 조회하지 않는다. 가설 생성에는 유사 RCA 보고서가 주입된다.
- **플레이북 업데이트 Search-First**: 구현 유지 — F8 플레이북 생성 시 유사도 0.86 이상의 기존 플레이북이 있으면 상세 절차를 로드한 뒤 LLM이 업데이트 여부를 판단하여 중복 생성을 방지한다. 상세를 로드할 수 없는 후보는 건너뛴다.
...
현재 플레이북은 생성 → Cohere Embed V4 임베딩 → S3 Vectors 인덱싱 → SNS 알림 포함까지 수행된다. 스코핑 단계에서는 더 이상 플레이북을 조회하지 않으며(ADR 0001로 보고서로 교체됨), 플레이북의 복구 절차(temporary_mitigation, permanent_remediation)를 자동 실행하는 경로는 향후 Remediation Agent에서 처리한다.

## Related

- [ADR agent/0001: 초기 스코핑 + RCA 보고서 유사도 검색](0001-initial-scoping-and-report-similarity.md) — 스코핑 단계의 유사도 검색은 보고서로 교체됨
