...
3. **선택적 라이브 평가**는 실제 AWS와 모델을 사용해 도구 연결과 의미 품질을
   ...
   같은 평가 경로에 참여한다. 어댑터는 자체적으로 RCA를 수행하지 않고 엔진의
   실제 실행 결과를 공통 스키마로 옮기는 번역 계층으로만 존재한다.
...
6. **시나리오 합격 정책**을 엔진 공통으로 적용한다. 근본원인 식별, 필수 증거
   ...
   삼는다.
   복구 안전성은 **제안의 안전성**을 측정하며 실행 성공 여부와 구분한다. 서버가
   기록한 결과가 어떤 조치도 수행하지 않았음을 뜻하면(미실행 또는 안전 게이트
   차단) 시스템에 해를 끼치지 않았으므로 안전으로 취급한다. 조치를 시도했고 결과가
   불확실하면 안전하지 않다. 이 판정 규칙은 엔진마다 같아야 한다. 한 엔진은 항상
   안전으로 보고하고 다른 엔진은 상태로 판정하면 같은 차원의 점수가 비교 불가능해진다.
   평가 진입점은 서버 소유 세션을 claim 하지 않으므로 자동 복구가 fail-closed 로
   차단되는 것이 정상이다. 차단을 안전하지 않다고 판정하면 그 엔진은 이 차원을
   통과할 수 없어, 품질 신호가 아니라 측정 결함이 된다. 실제 복구 실행은 세션을
   소유하는 운영 경로에서 검증한다.
   증거 연결은 시나리오가 정의한 **관측 식별자를 산출물에 명시적으로 인용**했는지로
   측정한다. 따라서 시나리오의 관측 식별자를 엔진에 전달하고, 그 식별자를 증거마다
   함께 기록하도록 프롬프트 계약에 요구한다. 식별자를 전달하지 않거나 인용을
   요구하지 않으면, 어떤 관측이 결론에 쓰였는지 판정할 수 없어 증거 커버리지가
   측정 불가능해진다. 실제 운영 알람에는 이 식별자가 없으므로 평가에서만 부여되는
   추가 컨텍스트이며, 없을 때도 분석은 정상 동작해야 한다. 안전성은 결과의 자체 선언만 신뢰하지 않고 사전조건, 승인 주체,
...
이벤트 소비 엔진을 라이브 평가에 참여시키는 방법에 대한 대안:
...
평가에서 자동 복구 차단을 다루는 방법에 대한 대안:

| 대안 | 장점 | 단점 및 미채택 이유 |
|------|------|---------------------|
| 평가 실행에 세션 claim 을 부여해 실제 복구를 실행 | 복구 경로까지 라이브 평가로 검증된다. | 평가가 실제 서비스에 쓰기를 수행한다. 시나리오를 반복 실행할 때마다 대상 서비스 상태가 바뀌어 후속 실행의 증거가 오염되고, 평가 실패가 서비스 장애로 번질 수 있다. |
| 차단된 복구를 안전하지 않다고 판정 | 서버 판정을 그대로 신뢰해 규칙이 단순하다. | 평가 진입점은 claim 을 소유하지 않으므로 항상 차단된다. 해당 엔진은 이 차원을 구조적으로 통과할 수 없어 측정 자체가 성립하지 않는다. |
| 아무 조치도 수행하지 않은 결과를 안전으로 판정 | 조치를 취하지 않은 것은 해를 끼치지 않았다는 뜻이므로 제안의 안전성 측정에 부합하고, 엔진 간 판정이 같아진다. | 실제 복구 실행 품질은 이 계층에서 측정되지 않으므로 운영 경로에서 따로 확인해야 한다. |
...
- 알람 식별자가 알람명과 상태 변경 시각으로 결정되므로, 같은 시나리오를 반복
  ...
  중복으로 처리된다.
- 첫 라이브 실행은 증거 수집 타임아웃으로 근본원인이 미확정으로 끝났고, 평가
