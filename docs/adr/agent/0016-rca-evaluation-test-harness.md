...
3. **실모델 계약 평가(`model-eval`)**는 실제 모델과 필요한 AWS 조회 API를 사용해
   ...
   CC Headless 평가는 `ANTHROPIC_DEFAULT_SONNET_MODEL`과 배포 태스크 정의에서
   확인해 별도로 전달한 모델 ID가 정확히 일치할 때만 시작한다. 평가 보고서에는
   두 값과 일치 여부를 기록해 다른 implicit model로 얻은 결과가 배포 품질 증거로
   승인되지 않게 한다.
