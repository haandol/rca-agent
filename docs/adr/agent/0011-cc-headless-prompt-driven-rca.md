...
9. **세션 상태 관리**: 기존 Fargate 스택과 동일한 DynamoDB 테이블에 세션을 기록한다. `engine` 필드로 실행 엔진을 구분한다 (`strands` vs `cc-headless`). SK 접두사에 엔진명을 포함하여 엔진별 트레이스를 분리한다. 상태 전이에 CANCELLED 가드를 적용하여 취소된 세션이 COMPLETED/FAILED로 전이되지 않도록 보호한다. `update_item`에 `attribute_exists(SK)` 조건을 추가하여 phantom HYPO 레코드 생성을 방지한다.
