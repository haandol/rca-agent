...
- 예산이 엔진마다 다르므로 같은 장애에 대해 두 엔진의 종료 시점이 갈린다. 한쪽이 부분 결과로 끝난 뒤 다른 쪽이 계속 탐색해 더 나은 결론에 도달할 수 있다
...
- [ADR agent/0011: CC Headless 오케스트레이션](0011-cc-headless-prompt-driven-rca.md) — 단일 장시간 실행으로 분석하는 엔진의 구조
