...
분석 완료 알림은 이제 **사람에게 승인을 요청하는 신호**이기도 하다. 실행은 사용자가 명시적으로 승인할 때만 시작되므로([ADR 0017](0017-playbook-execution-agent.md)), 이 알림을 받은 사람이 리포트를 열어 실행 여부를 판단한다. 알림이 전달되지 않으면 실행이 시작되지 않는다.

과거에는 이 알림이 자동 복구 워커의 진입 이벤트로도 쓰였고, 복구 결과가 별도 이벤트로 발행되어 두 이벤트를 필터로 구분해야 했다. 자동 복구가 폐지되면서 기계 소비자가 사라지고, 알림의 수신자는 사람과 대시보드만 남았다.
