...
모델 세대가 올라가면 호출 표면도 달라진다. 최신 세대는 sampling 파라미터를 받지
않고 사고량 제어를 별도 파라미터로 옮겼는데, 관리형 추론 API의 파라미터 지원은
모델 세대와 배포 시점에 따라 다르다. 문서상 지원되는 파라미터가 실제 배포에서
거부되는 경우가 있어, 모델을 올릴 때 호출 표면을 실측으로 확인하지 않으면 전체
파이프라인이 검증 오류로 실패한다.
...
- 모델 호출 파라미터는 대상 추론 플랫폼이 실제로 허용하는 집합만 사용해야 한다.
...
기본 모델은 **Claude Sonnet 5 세대**를 사용한다. 양쪽 엔진이 같은 세대를 쓰는 제약은
유지된다.

이 세대의 호출 표면에는 두 가지 제약이 있으며, 대상 추론 플랫폼에서 실측으로
확인한 바를 요구사항으로 고정한다.
...
두 제약은 모델 세대와 플랫폼 배포에 종속되므로, 모델을 올릴 때 최소 호출로 파라미터
허용 여부를 확인한 뒤 코드를 옮긴다.
