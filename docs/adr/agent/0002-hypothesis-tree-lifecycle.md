...
**구조화된 가설 트리 + LLM 기반 판단 + 서버 소유 상태 판정 + Beam Search +
Accepted Review Gate** 전략을 채택한다. 가설의 생성·우선순위 결정·검증·분기 네
단계가 공통 도메인 모델을 공유하며, 검증 루프는 우선순위 상위 N개만 선택적으로
검증한다. 매 검증 루프 진입 시 Accepted Review Gate가 기존 채택 가설을 리뷰한다.

LLM은 가설, 증거 요약, 신뢰도, 검증 원인 유형 후보를 제안한다. 두 분석 엔진의
서버는 동일한 규칙으로 상태, 탐색 진행, 근본원인 확정 여부를 판정한다. 모델이
상태나 다음 단계를 선언해도 서버 판정을 바꿀 수 없다.
...
- **초기 가설 집합**: 첫 생성 라운드는 루트 가설 3~5개를 만들며 모든 루트는 `PENDING`, depth 0이다. 범위를 벗어난 개수, 다른 초기 상태, 부모가 있는 루트, 0이 아닌 깊이는 저장 또는 완료가 거부된다.
- **상태 분류 권위**: 신뢰도 0.8 이상은 `CONFIRMED`, 0.3 이하는 `REJECTED`, 그 사이는 `NEEDS_INVESTIGATION`다. 모델이 다른 상태 버킷을 제안해도 저장된 상태는 신뢰도 규칙과 일치한다.
- **증거 없는 확정 금지**: 필수 증거가 있는 가설의 증거 수집이 실패했거나 증거 요약이 비면 `CONFIRMED`가 될 수 없다. 높은 신뢰도라도 증거 실패 가설은 최대 `NEEDS_INVESTIGATION`으로 남는다.
- **검증 원인 유형**: 생성·분기 단계의 원인 유형은 조사 힌트다. 검증 단계가 증거를 근거로 독립 판정한 유형만 최종 원인 유형이 된다. 초기 유형과 검증 유형이 달라도 검증 유형이 최종 결과에 남고, 초기 유형 일치를 완료 조건으로 요구하지 않는다.
- **루프 순서와 상한**: 검증 루프 번호는 1부터 연속하며 최대 3회다. 번호 누락, 역전, 4번째 루프는 거부되고 완료 세션에 포함되지 않는다.
- **전체 기각 재생성**: 전체 기각 시 최대 2회 재생성한다. 새 라운드는 이전 가설과 판정을 보존하고 새 트리 식별자를 추가한다. 재생성 뒤에도 이전 가설 ID와 판정 참조가 유효하며, 덮어쓰기로 과거 판정이 사라지지 않는다.
- **완료 전 종결**: 보고서 생성 전에 모든 가설은 `CONFIRMED`, `REJECTED`, `CLOSED` 중 하나다. 미확정 최선 후보도 `CLOSED`로 종결한 뒤 보고서가 이를 참조하며, `PENDING` 또는 `NEEDS_INVESTIGATION` 가설이 남은 실행은 완료되지 않는다.
...
- **전체 기각 후 재생성**: 검증 결과 모든 가설이 기각되면 가설 생성으로 루프백한다.
  재생성은 최대 2회이며 이전 생성 라운드와 판정을 보존한다. 재생성이 기존 가설
  산출물을 덮어쓰면 이전 validation 참조가 무효가 되므로 라운드 기록은 누적한다.
...
- **5 Whys 사고 프레임 (Toyota / AWS COE)**: 각 가설은 "증상에 대한 1차 '왜?'의 답 후보" 로 구성하여, 분기 단계에서 다시 "왜 그게 발생했는가?" 로 한 단계 더 내려갈 수 있어야 한다. 다음 가드레일을 프롬프트에 강제한다.
  ...
  동일 가드레일이 headless-codex 엔진의 `hypothesis-generation` skill·서브에이전트 프롬프트에도 적용된다([ADR 0011](0011-cc-headless-prompt-driven-rca.md)). 보고서 단계 5 Whys 작성 규칙은 [ADR 0007](0007-rca-report-generation.md) 참조.
...
- **원인 유형 독립 검증**: 가설 생성·분기 단계가 붙인 원인 유형은 조사 힌트일 뿐
  결론의 권위가 아니다. 검증 단계가 실제 수집 증거를 기준으로 원인 유형을 독립
  분류하고, 확정 판정과 함께 저장된 검증 원인 유형만 리포트와 플레이북의 입력이
  된다. 증거가 부족하거나 설명·증거와 유형이 일치하지 않으면 확정하지 않는다.
  초기 분류를 그대로 신뢰하면 오분류가 리포트의 근본 원인으로 굳어지고, 그 리포트를
  근거로 만든 플레이북이 잘못된 절차를 기술한다([ADR 0008](0008-playbook-generation.md)).
...
검증 루프가 종료되면(CONFIRMED 발견, 시간 예산 소진, 최대 루프 도달 등) PENDING 또는
NEEDS_INVESTIGATION 상태로 남은 가설을 CLOSED로 처리한다. REJECTED는 증거로
반증된 가설에만 사용한다. 미확정 최선 후보도 CLOSED 상태로 종결하되 보고서가 후보
식별자와 마지막 신뢰도를 참조한다. 두 엔진은 같은 서버 판정으로 모든 가설을
CONFIRMED/REJECTED/CLOSED 중 하나로 만든 뒤에만 보고서 생성으로 전환한다.
...
- 두 엔진의 상태 분류와 탐색 결과를 같은 서버 규칙으로 비교할 수 있음
...
- Headless Codex는 한 모델 실행 안에서 탐색 컨텍스트를 유지하면서도 서버 판정
  응답을 따라야 하므로 하네스와 상태 계약이 함께 유지되어야 함
