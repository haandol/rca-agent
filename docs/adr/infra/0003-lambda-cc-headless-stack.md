...
이 태스크는 **분석만 수행한다.** 과거에는 같은 런타임에 읽기 도구와 제한된 복구
쓰기 도구를 함께 제공하고 역할별 도구 허용 정책으로 경계를 세웠다. 복구가 사용자
승인 뒤의 별도 스택으로 분리되면서([ADR infra/0008](0008-playbook-execution-stack.md))
이 태스크에서 쓰기 도구가 사라졌고, 한 런타임 안에서 읽기·쓰기 노출 범위를 나눌
필요도 없어졌다.
