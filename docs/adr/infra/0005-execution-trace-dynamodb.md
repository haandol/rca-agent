...
RCA 에이전트(Strands)의 파이프라인은 9단계(스코핑→가설 생성→우선순위→증거 수집→검증→분기→보고서→플레이북→알림)로 구성되며, 검증 루프를 최대 3회 반복한다. 자동 복구(치유/검증)는 별도 Remediation 에이전트로 분리되어 있다(ADR agent/0012). 현재 DynamoDB에는 세션 수준 상태만 저장되어, 대시보드에서는 "현재 어떤 단계인지"와 최종 보고서만 확인 가능하다.
