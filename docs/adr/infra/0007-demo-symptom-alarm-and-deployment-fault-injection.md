...
활성 데모는 PostgreSQL 저장 컬럼 불일치 하나다. Strands의 올바른 원인 확정,
고정 런북, 사용자 승인, 실행 워커의 정상 이미지 롤백, 배포 수렴과 실제 저장 회복,
서버의 해결 판정까지를 필수 성공 경로로 삼는다. 회고는 해결 뒤 별도로 기록한다.
...
flowchart LR
    ...
    F --> E["실제 오류·스키마·배포 수집"] --> C["Strands 원인 확정"]
    C --> A["고정 런북·사용자 승인"] --> R["실행 워커 정상 이미지 롤백"]
