...
Vital Sensor의 심박수 중심 이벤트 저장이 실패할 때 실제 오류와 배포 변경으로 원인을 확정하고,
...
해결 판정, 장애 중 보존한 이벤트의 중복 없는 저장 완료가 모두 필요하다.
실행 증거 기반 회고·검증됨 승격은 해결 뒤 별도로 기록한다.

### Vital 입력과 미완료 이벤트

입력·보존 계약은 [Vital Sensor 저장](0004-rds-healthcare-deployment.md)을 따른다.
서비스 전체에서 초당 한 건의 이벤트를 생성하고, 정상 버전은 이벤트 v1의 `timestamp`와
v2의 `sampled_at`을 모두 물리 컬럼 `timestamp`에 저장한다. 결함 버전은 같은 v2 입력을
받지만 INSERT 컬럼 참조까지 `sampled_at`으로 바꾸어 실제 `42703` 오류를 낸다.
이벤트 스키마 버전과 앱 빌드 버전은 별개이며, 이름만으로 입력 호환성을 입증하지 않는다.

정상·결함·롤백 이미지는 같은 내구 입력 저장과 중복 방지 계약을 사용한다. 장애 중
기존 PostgreSQL에 보존된 이벤트는 같은 ID·측정 시각·값으로 간격을 늘리고 무작위 지연을
더해 재시도한다. 미완료 최대 86,400건에서 신규 생성을 멈추고 기존 이벤트는 버리지 않는다.
앱·DB 완전 중단 구간은 소급 생성하지 않는다. 롤백 호환성은 실제 v2 수신·저장과
미완료 이벤트 인계 증거로 확인하며, 해당 기능이 없는 과거 정상 이미지를 추정해 쓰지 않는다.
...
| 정상·장애·복구에 같은 이벤트 형식·생성 주기·처리 설정을 적용하고 재시도는 원래 이벤트 내용을 보존한다. | 새 이벤트는 서로 다른 ID·측정 시각을 가지며, 재시도 때문에 값이 바뀌거나 새 이벤트로 집계되지 않는다. 시작·완료·실패·진행 중 및 생성하지 못한 이벤트를 구분하고 소급 생성이나 몰아 보내기를 하지 않는다. |
...
미완료 입력 수용은 최종 측정 저장 성공이 아니다. 신규 입력·재시도·실제 SQL 실패·고유
커밋·미완료 건수를 구별하고, 중복 응답이나 대기열 기록만으로 저장 회복을 주장하지 않는다.
...
전체 데모 완료 확인은 이 실행 판정과 별도다. 장애 구간에 실제 보존된 이벤트 집합을
해당 실행과 원본 측정 내용에 연결하고, 롤백 후 각 이벤트가 한 번씩 저장되고 미완료로
남지 않았는지 확인한다. 개인정보·측정 값은 진단 증거에 노출하지 않는다. 신규 저장
성공이나 전체 미완료 건수 감소만으로 해당 집합의 처리가 끝났다고 추정하지 않는다.
확인 자료가 없으면 서버의 기존 해결 판정은 그대로 보존하되 전체 데모 완료로 표시하지 않는다.
이 집합을 나중에 유리하게 선택하지 않도록 새 Vital 시연은 장애 주입 전에 저장 세대와
시작 경계를 고정한다. 시작 경계가 없으면 새 장애를 주입하지 않는다. 정상 배포가 수렴한
뒤 종료 경계를 고정하며, 재검증은 같은 경계를 사용한다. 과거 기록에 없는 경계는 만들어
채우지 않고 기존 기록의 읽기·복원 경로를 유지한다.
