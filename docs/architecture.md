...
## 재사용할 플레이북과 이번 사고의 런북

플레이북은 같은 유형의 장애에서 다시 사용할 증상·메트릭·판단 기준·대응 지식입니다.
런북은 이번 사고의 대상·리전·구체 명령·순서·성공 기준입니다. 두 분석 엔진은
새 사고의 초안을 만든 뒤 S3 Vectors에서 유사도 0.80 이상 후보의 게시된 상세를 읽고,
현재 증거와 비교해 적용 가능성과 보강할 내용을 판단합니다.

비교 결과는 변경 제안, 변경 불필요, 유사 후보 없음, 적용 가능한 후보 없음,
검색·비교 실패를 구분해 남깁니다. 변경 제안에는 기준 개정본, 기존 내용과 제안 내용,
항목별 변경 이유와 증거가 함께 보존됩니다. 사용자가 반영하기 전까지 기존 공개 지식은
유지되며, 반영 전에 기준이 바뀌었으면 서버가 덮어쓰기를 거부합니다.
런북은 현재 사고의 증거로 새로 작성하므로 과거 대상이나 명령을 상속하지 않습니다.

```mermaid
sequenceDiagram
    participant Agent as 분석 엔진
    participant Search as S3 Vectors
    participant Store as 게시된 지식 저장소
    participant User as 운영자
    Agent->>Search: 유형·증상·메트릭으로 유사 검색
    Search-->>Agent: 후보 식별자와 유사도
    Agent->>Store: 후보의 현재 게시 상세 조회
    Store-->>Agent: 기준 개정본과 대응 지식
    Agent->>Store: 현재 증거와 비교한 변경 제안 보존
    User->>Store: 변경 전후 검토 후 반영 요청
    alt 기준 개정본이 그대로임
        Store->>Store: 지식과 반영 결과를 원자적으로 확정
        Store->>Search: 확정한 개정본 게시
    else 기준 개정본이 바뀜
        Store-->>User: 재비교 필요
    end
```

검색 게시가 실패하면 원본과 반영 결과는 남고 같은 개정본의 게시만 재시도합니다.
검색은 게시된 현재 개정본과 완료된 원본의 관계를 확인하며, 늦게 도착한 과거 게시를
현재 지식으로 사용하지 않습니다. 해결된 실행의 자동 회고는 계속 동작하고, 회고가
기준 지식을 바꾸면 대기 중인 제안에도 같은 최신 개정본 검사를 적용합니다.

대시보드의 플레이북 라이브러리는 장애 목록과 별도로 공개 지식을 조회합니다.
리포트는 요약 화면에서 영향·원인·짧은 5 Whys 사슬·다음 조치를 먼저 보여주고, 전문·증거·가설·타임라인·
런북·생성 참고 자료·변경 제안·회고는 같은 페이지의 모달로 엽니다. 참고 자료는 실제로
재사용한 플레이북의 당시 내용과 생성·판단에 사용한 분석 입력·인용 증거 및 활용
용도를 보여주며, 검색에만 걸린 후보는 생성 출처로 표시하지 않습니다.
**2단계는 열람 방식**이며 원문이나
상세 항목을 삭제하지 않습니다. 모달 안에서 자료를 전환하고 닫으면 원래 위치로
돌아옵니다. 지식 반영과 런북 실행 승인은 별개이며, 실행 승인에는 현재 명령 전체의
검토가 필요합니다.
...
| **구분**          | DynamoDB `engine` 필드: `strands` vs `headless-codex`               |
...
graph TB
    ...
    subgraph Compute["에이전트 실행 (Dual-Stack)"]
        ...
        ECS_CC["ECS Fargate<br/>Headless Codex (main.py)"]
    end
    ...
    subgraph LLM["LLM 추론"]
        ...
        BEDROCK_CC["Amazon Bedrock Runtime<br/>Global gpt-5.6-sol · high<br/>(Headless Codex 오케스트레이터)"]
    end
...
## Agent Pipeline — Fargate (Headless Codex 오케스트레이터)
...
실행은 분석과 별개의 워커입니다. 진입점은 `python -m headless_codex.execution_main`이며,
...
agent/headless-codex 양쪽 패키지는 Hexagonal Architecture를 적용하여 비즈니스 로직과 인프라를 분리합니다.

```
패키지 구조 (agent, headless-codex 공통):
├── ports/                    # 인터페이스 계층
...
- **세션 추적**: 동일 DynamoDB 테이블, `engine: 'headless-codex'` 필드로 구분

### Playbook Execution (Headless Codex 실행 워커)

- **같은 이미지, 다른 진입점**: `python -m headless_codex.execution_main`. 하나의 하네스를 두 진입점으로 나눔
...
| Component     | Fargate Stack (Strands)                                                          | Fargate Stack (Headless Codex)                          |
| ------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------- |
...
| ---------------- | --------------------------------------------------------------------------------------- |
| 실행 엔진        | Codex CLI headless — 분석 워커와 동일 이미지, `headless_codex.execution_main` 진입점 |
