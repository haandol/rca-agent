...
git diff --stat <배포태그>..HEAD -- packages/headless-codex  # Codex 분석 워커와 실행 워커 공용
```
...
pnpm --filter infra run deploy:service -- headless-codex  # Codex 분석 워커
...
저장 실패 알람은 장애 적용 후 최소 **150초(2분 30초)** 동안 관측한다. 드라이버는
validation child가 시작된 뒤의 경과 시간과 시작·종료 시각을 기록하며, 150초 전에
성공 코드로 종료해도 검증 실패로 처리하고 cleanup을 수행한다. 장애 적용 전
red-herring 대기 시간은 이 구간에 포함하지 않는다. 시간 조건만으로 알람·분석·복구
검증을 대신할 수 없으며, 아래 단계의 실제 결과도 확인해야 한다.
...
Strands의 가설별 원본 증거는 `rca/<RCA_ID>/evidence/`에 보존된다. Headless Codex의
...
> **승인 대상 확보가 이 실측의 병목이다.** headless-codex 는 예산 소진(60분)이나 산출물 스키마
