...
**`PLAYBOOK_TOP_K=3`이 후보 수를 제한한다.** 인덱스가 49개 항목을 보유하고 유사한
플레이북이 여럿 쌓인 상태에서, 임계값을 넘는 개정본이 상위 3위 밖으로 밀릴 수 있다. 이번
실측에서 그 경우를 만나지는 않았지만 축적이 늘면 실현될 조건이다.
...
CC는 이번에도 CLI 30분 한도로 `FAILED`했다 — 알려진 병목이며 갭 B의 라이브 확인은 여전히
남아 있다.
