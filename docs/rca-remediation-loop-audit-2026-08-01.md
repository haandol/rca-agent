...
| 대상 | 결과 |
|------|------|
| `packages/agent` 테스트 | **522 passed** (기존 516 + 신규 계약 테스트) |
| `packages/cc-headless` 테스트 | **536 passed** (기존 508 + 신규 계약 테스트) |
...
**개정본 우선 조회(갭 C)와 CC의 검색·병합(갭 B)은 여전히 오프라인 계약으로만 검증된
상태다.** 위 두 결함 때문에 라이브에서 그 경로가 실행되지 않았다. 수정 후 재실측이
필요하며, 확인할 것은 하나다: **같은 증상을 두 번 주입했을 때 두 번째 분석이 첫 회고가
승격한 절차를 참조하는가.**

CC는 두 회차 모두 CLI 30분 한도로 `FAILED`했다 — 런북에 기록된 알려진 병목이며 이번 변경과
무관하지만, 그래서 CC 경로는 한 번도 실행되지 않았다.

> **인덱스 재적재가 필요하다.** 임베딩 입력 필드를 바꿨으므로 기존 레코드는 옛 텍스트로
> 임베딩된 상태다. 새 쿼리와 공간이 어긋나 검색되지 않으므로, 전환 이전 플레이북의 축적을
> 이어가려면 재적재해야 한다.
