...
실행은 사람이 대시보드에서 절차를 읽고 승인 버튼을 누를 때만 시작된다. 승인은
`POST /api/executions`가 **실행 요청 큐에 메시지를 발행**하는 것으로 표현된다.

승인 여부를 플래그나 코드 조건으로 두지 않고 큐 메시지로 둔 이유가 있다. 조건으로 두면
게이트는 그 조건을 검사하는 코드 한 줄에 의존한다. 그 검사가 빠진 경로가 하나 생기면 게이트가
없는 것과 같아진다. 큐로 두면 게이트는 **경로 자체의 성질**이 된다. 실행 스택에는 SNS 구독도
이벤트 규칙도 없고, 유일한 입구가 대시보드만 발행하는 큐다. 즉 큐에 메시지가 있다는 사실이
곧 사람이 승인했다는 사실이다. 승인 없이 실행이 기동될 경로는 존재하지 않는다.
...
승인 API는 워커가 어차피 거부할 요청을 발행하지 않는다. 분석 세션이 `COMPLETED`가 아니면
거부한다 — 끝나지 않은 분석 뒤에는 승인할 리포트가 없다. 실행 식별자는 요청 내용에서
결정론적으로 파생하므로 같은 승인이 재전달되어도 같은 식별자가 나오고, claim이 중복 실행을
걸러낸다.
...
실행 에이전트는 저장된 리포트를 읽어 `execution_steps`를 순서대로 수행한다. 알림 payload가
아니라 저장된 리포트를 직접 읽는데, 전달 과정에서 잘린 절차가 실행되지 않게 하기 위해서다.
회고가 갱신한 플레이북이 있으면 그것이 원본 플레이북보다 우선한다.
...
```mermaid
stateDiagram-v2
    [*] --> PENDING_APPROVAL: 승인 요청이 큐에 발행됨
...
실행 시작 시점에 **갱신 전 플레이북 사본을 S3에 보존**한다. 회고가 원본을 덮어쓰므로 사본이
없으면 갱신 diff의 기준이 사라진다.

### 승격 — 초안이 검증됨이 되는 유일한 지점

회고가 갱신을 반영하면 `verification_status`가 `DRAFT`에서 `VERIFIED`로 올라간다. 전이는
**한 방향뿐이고 되돌아오지 않는다.** 한 번 실행으로 입증된 절차가 이후 분석이나 병합으로
초안으로 떨어지면 이 값이 "실행으로 입증되었는가"를 더 이상 뜻하지 않는다.
...
승격은 개정본과 검색 인덱스 **양쪽에 함께** 반영된다. 다음 실행은 개정본을 읽고 다음 RCA의
보강은 인덱스를 읽으므로, 한쪽만 갱신하면 승격이 한 경로에서만 보인다. 같은 이유로 이후
보강도 이 값을 낮추지 않는다 — 병합이 축적을 되돌리지 않는다는 규칙이 검증 상태에도
적용된다.

### 루프가 닫힌다

갱신된 플레이북은 **같은 `playbook_id`를 유지**한다. 새 식별자를 만들면 축적이 갈라지고
"같은 증상의 플레이북"이 여러 개가 된다. 같은 식별자를 유지하므로, 다음에 같은 리포트가 실행될
때 실행 에이전트가 읽는 것은 원본이 아니라 이 갱신본이다. **갱신된 플레이북이 다음 실행의
근거가 되는 지점이 여기다.**
