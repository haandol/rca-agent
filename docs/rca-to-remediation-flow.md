...
flowchart TB
    ...
    WRK["플레이북 실행 에이전트<br/>headless_codex.execution_main"]
...
실행은 분석과 별개의 워커다. 진입점은 `python -m headless_codex.execution_main`이며 분석
...
| | 분석 | 실행 |
|---|------|------|
| 진입점 | `main.py` (엔진별) | `python -m headless_codex.execution_main` |
