...
> 경로(갭 B)다. 그 선결 조건이었던 예산 부족은 진단해 고쳤다 — 30분 예산의 81~98%를 쓰던
> 완주 회차에 여유를 주려고 60분으로 올렸고, 함께 옮겨야 하는 값 두 개도 정합시켰다
> ([§8](./rca-remediation-loop-audit-2026-08-01.md)). **CC 완주를 기다려 갭 B를 확인하는
> 것이 다음 회차의 목적**이다.
...
Strands는 약 10~16분, CC는 관측된 완주 구간이 24~30분이고 예산은 60분이다.

> **어느 엔진이든 `COMPLETED` + 절차 있는 리포트 하나면 갭 A·C 실측이 된다.** 다만 CC
> 경로의 검색·병합(갭 B)은 CC가 완료해야 확인되므로, 그것까지 보려면 CC 완주를 기다린다.
>
> **CC 한 회차는 최대 60분을 점유한다.** 그 사이 도착한 알람은 대기하고, 예산을 다 쓴
> 회차의 대기도 다음 알람에 전가된다. 진행 중인 세션이 있으면 새 알람이 세션을 만들지
> 않으므로, 재주입 전에 이전 세션의 `state`를 확인한다.
>
> CC 소요 시간과 그 값이 왜 60분인지는
> [§8](./rca-remediation-loop-audit-2026-08-01.md)이 보유한다.
