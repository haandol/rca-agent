...
graph LR
    ...
    subgraph CcStack["🟧 Headless Codex Stack (ECS Fargate)"]
        ...
        L_MODEL["Global profile<br/>gpt-5.6-sol · high"]
...
flowchart TD
    ...
    subgraph CCProcess["Codex CLI (서브프로세스)"]
        CC["codex exec --json --ephemeral<br/>Global gpt-5.6-sol · high"]
    ...
    subgraph MCPServers["MCP 서버 (전문 에이전트별 허용)"]
...
- **회고가 갱신을 반영하면 현재 실행 절차 내용에 따라 검증 상태가 정해집니다.** 절차가 그대로 이슈를 해소했거나 메타데이터만 바뀌면 `VERIFIED`로 승격·유지됩니다. `execution_steps`가 추가·교정되면 새 절차는 아직 실행되지 않았으므로 `DRAFT`이며, 다음 해결 실행과 회고 뒤 다시 승격됩니다. 회고가 실패하면 상태도 갱신되지 않습니다. 대시보드 리포트 화면이 초안/검증됨을 표기하므로 운영자는 승인 전에 현재 절차가 실행으로 입증된 것인지 구분할 수 있습니다.
...
| Codex CLI 0초 완료           | HOME 디렉토리 미설정                                                                 | 컨테이너 환경변수 HOME=/tmp 확인                                                                                                                                                                                  |
