...
- **검증** (라이브, 배포 기반 주입):
  ...
  - Healthcare 42 passed, infra 30 passed, agent 502, cc-headless 486
...
통합 테스트를 추가한다. 실행 안전 경계 변경은 다음 부정 테스트를 반드시 포함한다.

> 이 목록은 자동 복구가 폐지되고 사용자 승인 기반 플레이북 실행으로 대체되면서
> 갱신됐다. reset 도구는 더 이상 존재하지 않으므로 아래는 현재 경계를 기준으로 한다.

- 사용자 승인 없이 실행이 시작되지 않는다.
- 파괴적 액션과 판정 불가 명령이 차단되고 그 사실이 증거에 남는다.
- 미확정 원인의 플레이북에는 실행 절차가 없다.
- claim을 잃거나 취소된 실행은 상태, 증거, 리포트를 확정하지 않는다.
- 관측으로 확정할 수 없는 결과는 해결로 기록되지 않는다. 관측 부재는 미해결이다.
- 미해결·실패한 실행은 회고로 플레이북을 갱신하거나 검증됨으로 승격하지 않는다.
