...
- **`packages/agent`** – Strands Agents SDK 기반 RCA 에이전트 (Fargate). 12단계 파이프라인(스코핑 → 가설 생성 → 우선순위 → 증거 수집 → 검증 → 분기 → 종료 판단 → 보고서 → 플레이북 → 자동 복구 → 복구 검증 → 알림)을 실행합니다. Planning 티어(Sonnet 4.6 + adaptive thinking)와 Execution 티어(Haiku 4.5)의 2-tier 모델 아키텍처를 사용합니다.
...
- **`packages/infra`** – AWS CDK 인프라. ECS Fargate, Lambda Container, SNS/SQS, S3, S3 Vectors, DynamoDB, VPC/PrivateLink, Cloud Map, Bedrock 접근 등 전체 인프라를 코드로 관리합니다.
